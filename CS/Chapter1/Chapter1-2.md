...
> Keyword : 멀티프로세스, 멀티스레드, 콘텍스트 스위칭, 경쟁상태, 임계영역, 뮤텍스, 스핀락, 세마포어
> Keyword : 교착상태, 스레드 안전, IPC, 좀비 프로세스, 고아 프로세스
...
#### 교착 상태 막으려면
- 4가지 필요 충분 조건 중에 하나 제거하면 됨
- 상호배제 부정 : 여러 프로세스가 동시에 하나의 공유 자원을 사용할 수 있게 함
- 점유와 대기 부정 : 프로세스 실행 전에 필요한 모든 자원 할당해서 프로세스 대기 없앰. 혹은 프로세스가 자원을 점유하지 않은 상태에서만 자원 요구하게 함
- 비선점 부정 : 자원을 점유한 프로세스가 다른 자원을 요구할 때 점유한 자원을 반납하게 함
- 환형 대기 부정 : 자원을 선형 순서로 정렬해 고유 번호 할당. 각 프로세스에서 요구할 수 있는 번호 방향 정해서 한 쪽 방향으로만 자원 요구하게 함.
...
- 원자 연산(atomic operation) : 공유 자원에 접근할 때 원자 연산이나 원자적으로 정의된 연산을 이용해 연산 도중에 다른 스레드 접근할 수 없게 함.
- 재진입성(reentrancy) : 특정 함수를 하나의 스레드에서 실행 중일 때 다른 스레드가 해당 함수를 실행해도 각 스레드에 올바른 결과가 나올 수 있게 함.
- 스레드 지역 저장소(thread local storage) : 각 스레드에서만 접근할 수 있는 저장소를 사용해서 공유되는 자원 줄임.
* 원자 연산 : '연산했다'와 '연산 안 했다' 두 가지만 존재하는 연산


### IPC (Inter Process Communication)
- 프로세스 간에 자원을 공유하는 방식
- 프로세스는 고유한 메모리 영역을 갖기 때문에 프로세스 간 자원을 공유해야 할 때 IPC 해야 함

#### IPC의 대표적인 종류
1. 공유 메모리 : 프로세스 간 공유 가능 메모리를 구성해서 자원을 공유하는 방식. 동기화 문제 발생 가능.
2. 소켓 : 네트워크 소켓을 이용하는 프로세스 간 통신. 외부 시스템과도 이용 가능. 클라이언트와 서버 구조로 자원 주고받음.
3. 세마포어 : 접근하는 프로세스를 제어해 공유 자원 관리
4. 파이프 : FIFO 형태의 메모리인 파이프를 이용해서 프로세스 간 자원을 공유하는 방식. 파이프는 단방향 통신만 지원해서 읽기 또는 쓰기 중 하나만 가능. 양방향 통신하려면 읽기 파이프, 쓰기 파이프 각각 생성.
![파이프를 이용한 IPC](./src/1_25.png)
5. 메시지 큐 : FIFO 형태의 큐 자료구조로 프로세스 간 메시지 주고 받는 방식
![메시지큐를 이용한 IPC](./src/1_26.png)


### 좀비 프로세스와 고아 프로세스
#### 좀비 프소세스
- 자식 프로세스가 종료되었지만 부모 프로세스가 자식 프로세스의 종료 상태를 회수하지 않았을 경우에 남겨진 자식 프로세스
- 자식 프로세스 종료될 때 부모 프로세스에 SIGCHLD라는 시그널 보내면 부모 프로세스에서 wait() 함수(시스템콜)를 호출해서 자식 프로세스의 상태 정보 받고 자원을 회수
- 이 때, 자원 회수가 실패 시 좀비 프로세스 생기게 됨
- 좀비 프로세스가 쌓이면 자원 낭비

#### 고아 프로세스
- 부모 프로세스가 자식 프로세스보다 먼저 종료되는 경우에 자식 프로세스
- 해결법 : 자식 프로세스의 부모 PID를 init 프로세스(부팅 시 가장 먼저 실행되는 프로세스)의 PID인 1로 바꿔준다.
- 이렇게 하면 고아 프로세스의 부모 프로세스는 init 프로세스가 됨.
- 이후, 고아 프로세스가 작업 종료 시 init 프로세스가 고아 프로세스의 자원 회수
